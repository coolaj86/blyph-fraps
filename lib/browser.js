  ...
  */
  ...
  function getProviders(book) {
    ...
    function onProviderDataHttp(retailSales, trade, retailBuyBacks, need, unsorted) {
      ...
      retailSales = cbPrices((retailSales[2]||{}).result).filter(excludeRentals);
      ...
      retailBuyBacks = cbBuyBackPrices((retailBuyBacks[2]||{}).result);
    ...
    function getProviderData() {
      console.log('getProviderData', book);
      if (book.downloadInProgress && (now - book.downloadInProgress > 15 * 1000)) {
        console.log('waiting on download in progress');
        return;
      }

      console.log('downloading afresh...');
