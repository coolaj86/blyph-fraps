...
(function () {
  ...
  */
  ...
  function saveBooklist() {
    ...
    fullBooklist.userToken = bauth.user.userToken;
    // TODO update existing records with nickname
    fullBooklist.nickname = bauth.user.nickname;
    ...
    request({
        "method": "POST"
      , "href": "/booklist/" + bauth.user.userToken
  ...
  function showTraders(traders) {
    ...
    traders.forEach(function (trader) {
      ...
      traderHtml.find('.js-person img').attr('src', 'http://www.gravatar.com/avatar/' + gravatar + '?s=50&r=pg&d=identicon');
  ...
  function showConsumers(consumers) {
    ...
    consumers.retailBuyBack.forEach(function (consumer) {
      ...
      onlineStores.find('.js-ols-buy-sell')
        ...
        ;
      onlineStores.find('.js-result-for-price').html("Base price &#43; shipping $" + consumer.prices.sort()[0]);
  ...
  function showProviders(providers) {
    ...
    providers.retailSale.forEach(function (provider) {
      ...
      onlineStores.find('.js-result-for-price').html('$' + Number(provider.total_price).toFixed(2));
      storesArea.append(onlineStores);
    });
  }
  ...
  function create(app) {
    ...
    (function () {
      ...
      $("body").delegate("#js-search", "keyup", respondon.keyup);
  ...
  function listUploads() {
    ...
    function getBooklistHttp() {
      ...
      request({
          href: "/booklist/" + bauth.user.userToken + "?_no_cache_=" + new Date().valueOf()
      /*
        , headers: {
  ...
  function onSuccessfulLogin() {
    ...
    $(".js-load-booklist a").attr('href', href + '#/?userToken=' + bauth.user.userToken);
    $("#js-fb-unique-link").attr('href', 'blyph.com/#/?referredBy=' + bauth.user.userToken);
    $('#js-saveyourinfo').slideUp();
    $('#js-logout').show();
    $('.js-username').text(bauth.user.nickname);
    // TODO
    if (false) { Social.init(bauth); }
    listUploads();
  }
  ...
  domReady(function () {
    $('body').delegate('.js-button-want2', 'click', function (ev) {
      ev.stopPropagation();
      addBook($(this).closest('.js-item'), false, true);
    });
    $('body').delegate('.js-button-list2', 'click', function (ev) {
      ev.stopPropagation();
      addBook($(this).closest('.js-item'), true, false);
    });
    $('body').delegate('.js-button-keep2', 'click', function (ev) {
      ev.stopPropagation();
      addBook($(this).closest('.js-item'), true, true);
    });
    $('body').delegate('.js-button-ignore2', 'click', function (ev) {
      ev.stopPropagation();
      addBook($(this).closest('.js-item'), false, false);
    });
    ...
    $('#js-person-matches').delegate('.js-ppl-send-message', 'click', function (ev) {
      ...
      message.note = 
          "Hello firebugger / wiresharker."
        + " Yes, we plan on fixing this bug soon."
        + " Just trying to release, y'know."
        ;
  ...
  domReady(function () {
    ...
    location.hash = '';
    bauth.attemptLogin(function () { $(onSuccessfulLogin); });
