...
(function () {
  ...
  */
  ...
  function saveBooklist(cauth) {
    var booklist
      ;

    // TODO save guest user by a different token
    if (!cauth.user.userToken) {
      return;
    }
    ...
    fullBooklist.userToken = cauth.user.userToken;
    // TODO update existing records with nickname
    fullBooklist.nickname = cauth.user.nickname;
    ...
    request({
        "method": "POST"
      , "href": "/booklist/" + cauth.user.userToken
  ...
  function listUploads(cauth) {
    ...
    function getBooklistHttp() {
      ...
      request({
          href: "/booklist/" + cauth.user.userToken + "?_no_cache_=" + new Date().valueOf()
      /*
        , headers: {
  ...
  function onUserCreate(err, cauth) {
    ...
    //if (cauth.user.virgin) {
      $('#js-import-prompt').show();
    //}
    onUserLogin(err, cauth);
  }

  function onUserLogin(err, cauth) {
    var href
      ;


    cauth.user.referrerId = cauth.user.userToken.substr(14, 8);

    href = $(".js-load-booklist a").attr('href');
    $(".js-load-booklist a").attr('href', href + '#/?userToken=' + cauth.user.userToken);
    $("#js-fb-unique-link").attr('href', 'blyph.com/#/?referredBy=' + cauth.user.userToken);
    $('#js-saveyourinfo').slideUp();
    $('#js-logout').show();
    $('.js-username').text(cauth.user.nickname);
    // TODO
    if (false) { Social.init(cauth); }
    listUploads(cauth);
  }
  ...
  domReady(function () {
    ...
    location.hash = '';
    bauth.attemptLogin(function (err, cauth) { $(function () {
      onUserLogin(err, cauth);
    }); });
