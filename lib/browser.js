...
jQuery(function () {
  ...
  function jqForEach(callback, thisArg) {
    /*jshint validthis:true*/
    var jqArr = this
      ;

    jqArr.each(function (i, item) {
      if (thisArg) {
        callback.call(thisArg, item, i, jqArr);
      } else {
        callback(item, i, jqArr);
      }
    });
  }

  if (!$('body').forEach) {
    $.fn.forEach = jqForEach;
  } else {
    console.log("jquery finally has a forEach!!!");
  }
});
