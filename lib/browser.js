  ...
  */
  ...
  function saveBooklist(curUser) {
    ...
    if (!curUser.userToken) {
      console.warn('[saveBooklist] no userToken!');
      return;
    }
    ...
    request({
      ...
      if (err) {
        console.log('[saveBooklist]', err);
      } else {
        console.log('[saveBooklist]', data);
      }
    });
  }
  ...
  function showConsumers(consumers) {
    ...
    // find the best paying consumer offer
    function sortByPrice(a, b) {
      var diff = parseFloat(b['@value'], 10) - parseFloat(a['@value'], 10)
        ;
      return diff;
    }
    function sortConsumerByPrice(a, b) {
      return b.prices.sort(sortByPrice)[0]['@value'] - a.prices.sort(sortByPrice)[0]['@value'];
    }

    consumers.retailBuyBack.sort(sortConsumerByPrice).forEach(function (consumer) {
      var onlineStores
        ;
      ...
      onlineStores.find('.js-result-for-price').html('$' + consumer.prices.sort(sortByPrice)[0]['@value']);
  ...
  function listUploads(curUser) {
    ...
    function onBooklistHttp(err, ahr, data) {
        if (err) {
          console.error('[onBooklistHttp]', err);
  ...
  domReady(function () {
    ...
    bauth.init(jsonStorage, { onAccount: onUserLogin });
