...
(function () {
  ...
  */
  ...
  function onSuccessfulLogin() {
    ...
    $(".js-load-booklist a").attr('href', href + '#/?userToken=' + currentUser.userToken);
    $("#js-fb-unique-link").attr('href', 'blyph.com/#/?referredBy=' + currentUser.userToken);
    $('#js-saveyourinfo').slideUp();
    $('#js-logout').show();
    $('.js-username').text(currentUser.nickname);
    // TODO
    if (false) { showSocial(); }
    listUploads();
  }
  ...
  domReady(function () {
    ...
    $('#js-person-matches').delegate('.js-ppl-write-message', 'click', function (ev) {
      ...
      msg.bookTitle = $('#js-imported-booklist .item .title').text().trim();
    ...
    $('#js-person-matches').delegate('.js-ppl-send-message', 'click', function (ev) {
      ...
      message.bookTitle = $('#js-imported-booklist .item .title').text().trim();
  ...
  domReady(function () {
    ...
    DBG.displayItemTemplate = String($("#js-imported-booklist-tpl").html());
    onlineStoresTpl = String($('#js-online-matches-tpl ul').html());
    
    run();

    attemptLogin();

    if (!semver || semver.patch < curVer.patch) {
      users = jsonStorage.get('users') || {};
      if (currentUser.userToken) {
        users[currentUser.userToken] = currentUser;
      }
    ...
    // What the premature optimization?
    // BUG (jquery) somehow this remove below occurs before the .html() above
    //setTimeout(function () {
      $("div.item").remove();
